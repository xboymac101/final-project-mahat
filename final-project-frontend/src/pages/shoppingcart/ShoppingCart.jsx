...
useEffect(() => {
    ...
    axios
      ...
      .then((res) => {
        ...
        if (!phone_number || !address) {
          setShowPopup(true); // only show if missing
        }
      })
      .catch((err) => {
  ...
  function handleIncrease(book_id, currentAmount, maxCount, type) {
    if (currentAmount >= maxCount || processing) return;
    setProcessing(true);
    axios
      .post("/api/cart/add", { book_id, amount: 1, type }, { withCredentials: true })
      .then(() => {
        fetchCart();
        setProcessing(false);
      })
  ...
  axios.get("/api/auth/info", { withCredentials: true })
    .then((res) => {
      ...
      if (!phone_number || !address) {
        ...
        // redirect to PayPal Sandbox or your payment page
        window.location.href = "/payment"; // or use React Router navigate()
      }
    })
    .catch((err) => {
